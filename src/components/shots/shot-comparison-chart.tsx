...
export function ShotComparisonChart({ shot, shots }: { shot: ShotWithBean; shots: ShotWithBean[] }) {
  ...
  const current: Point = { id: shot.id, date: formatDate(shot.shot_at), name: shot.beans?.name ?? "Dieser Shot", time: shot.extraction_seconds, ratio: currentRatio, taste: shot.taste ?? "unknown", current: true };
  ...
  return <div className="h-[238px] w-full" aria-label="Shot-Vergleichsdiagramm">
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart margin={{ top: 12, right: 8, bottom: 2, left: -18 }}>
        ...
        <XAxis type="number" dataKey="time" domain={[xMin, xMax]} tick={{ fontSize: 11, fill: "#8D7E74" }} unit=" s" />
        <YAxis type="number" dataKey="ratio" domain={[yMin, yMax]} tick={{ fontSize: 11, fill: "#8D7E74" }} tickFormatter={(value) => `1:${value.toFixed(1)}`} />
        ...
        <Tooltip cursor={{ stroke: "#C8804F", strokeDasharray: "3 3" }} content={({ active, payload }) => {
          ...
          return active && point ? <div className="rounded-xl border bg-white p-2 text-xs leading-5 shadow-lg"><strong>{point.current ? "Dieser Shot" : point.name}</strong><p>{point.date} · {formatTime(point.time)}<br />{formatRatio(point.ratio)}</p></div> : null;
        }} />
...
function toPoint(shot: ShotWithBean): Point | null { const ratio = brewRatio(shot.final_yield_grams, shot.dose_grams); return shot.extraction_seconds !== null && ratio !== null ? { id: shot.id, date: formatDate(shot.shot_at), name: shot.beans?.name ?? "Shot", time: shot.extraction_seconds, ratio, taste: shot.taste ?? "unknown", current: false } : null; }
