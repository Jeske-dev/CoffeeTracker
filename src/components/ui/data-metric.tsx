...
export function DataMetric({
  ...
  return <div className={cn("min-w-0", centered && "text-center", className)}>
    <span className={cn("flex items-center gap-1.5 text-xs text-[var(--dialed-text-muted)]", centered && "justify-center", labelClassName)}>
      {Icon && <Icon aria-hidden="true" className={cn("size-3.5", iconClassName)} />}
      {label}
    </span>
    <strong className={cn("mt-1.5 block text-[13px]", truncateValue && "truncate", valueClassName)}>{value}</strong>
  </div>;
}
