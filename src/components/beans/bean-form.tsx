  ...
  return <form onSubmit={handleSubmit(submit)} className="grid min-h-0 grid-rows-[minmax(0,1fr)_auto]">
    ...
    <div className="scrollbar-none min-h-0 overflow-y-auto px-[18px] py-5 pb-6">
      <div className="mx-auto max-w-[680px]">
        ...
        <SectionCard title="Besonderheiten" icon={BadgeInfo} className="mb-3"><label className="flex min-h-12 items-center justify-between rounded-[14px] bg-[var(--dialed-surface-subtle)] px-3 text-xs">Entkoffeiniert<Switch checked={decaf} onCheckedChange={(value) => setValue("isDecaf", value)} /></label>{archiveAction && <div className="mt-3 border-t pt-3">{archiveAction}</div>}</SectionCard>
      </div>
    </div>
