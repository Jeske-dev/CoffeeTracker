...
export function BeanForm({ userId, bean, archiveAction }: { userId: string; bean?: Bean; archiveAction?: React.ReactNode }) {
  ...
  return <form onSubmit={handleSubmit(submit)} className="grid min-h-0 grid-rows-[minmax(0,1fr)_auto]">
    ...
    <div className="scrollbar-none min-h-0 overflow-y-auto px-6 py-6 pb-8">
      <div className="mx-auto max-w-[680px]">
        ...
        <SectionCard title="Röstung" description="Herkunft, Datum und Aufbereitung" icon={Flame} className="mb-3"><div className="grid gap-4 sm:grid-cols-2"><Field label="Röstdatum" icon={CalendarDays} error={errors.roastDate?.message}><NativeDateInput label="Röstdatum" value={roastDate} registration={register("roastDate")} onClear={() => setValue("roastDate", "", { shouldValidate: true })} /></Field><Field label="Röstgrad" icon={Flame}><select className="h-11 w-full border-0 border-b bg-transparent px-0 text-sm outline-none focus:border-b-2 focus:border-black" {...register("roastLevel", { setValueAs: (value) => value === "" ? null : value })}><option value="">Nicht angegeben</option><option value="light">Hell</option><option value="medium_light">Mittel-hell</option><option value="medium">Mittel</option><option value="dark">Dunkel</option></select></Field><Field label="Herkunft" icon={MapPin}><Input {...register("origin")} placeholder="Colombia" /></Field><Field label="Aufbereitung" icon={Sparkles}><select className="h-11 w-full border-0 border-b bg-transparent px-0 text-sm outline-none focus:border-b-2 focus:border-black" {...register("process")}><option value="unknown">Nicht angegeben</option><option value="washed">Washed</option><option value="natural">Natural</option><option value="honey">Honey</option><option value="anaerobic">Anaerobic</option></select></Field></div></SectionCard>

        <SectionCard title="Einkauf" description="Optional für Kosten und Frische" icon={ShoppingBag} className="mb-3"><div className="grid gap-4 sm:grid-cols-3"><Field label="Kaufdatum" icon={CalendarDays}><NativeDateInput label="Kaufdatum" value={purchaseDate} registration={register("purchaseDate")} onClear={() => setValue("purchaseDate", "")} /></Field><Field label="Packung" icon={Package}><Input type="number" step="0.1" placeholder="250 g" {...register("packageGrams", { setValueAs: (value) => value === "" ? null : Number(value) })} /></Field><Field label="Preis" icon={Euro}><Input type="number" step="0.01" placeholder="14,90 €" {...register("priceEuros", { setValueAs: (value) => value === "" ? null : Number(value) })} /></Field></div></SectionCard>
    ...
    <footer className="z-10 flex gap-2 border-t border-black bg-white px-6 pt-3 pb-[calc(14px+env(safe-area-inset-bottom))]"><Button type="button" variant="secondary" onClick={() => router.back()} className="h-12 flex-1">Abbrechen</Button><Button disabled={pending} type="submit" className="h-12 flex-1 bg-black text-white">{pending ? "Speichert …" : "Bohne speichern"}</Button></footer>
  </form>;
}
...
function NativeDateInput({ label, value, registration, onClear }: { label: string; value: string; registration: UseFormRegisterReturn; onClear: () => void }) { return <div className="relative"><input type="date" aria-label={label} className="h-11 w-full cursor-pointer border-0 border-b bg-transparent px-0 pr-9 text-sm outline-none focus:border-b-2 focus:border-black" onClick={(event) => event.currentTarget.showPicker?.()} {...registration} />{value && <button type="button" aria-label={`${label} löschen`} title="Nicht angegeben" onClick={onClear} className="absolute top-1/2 right-0 grid size-8 -translate-y-1/2 place-items-center border-l text-sm leading-none text-[var(--dialed-text-muted)] hover:bg-black hover:text-white">×</button>}</div>; }
